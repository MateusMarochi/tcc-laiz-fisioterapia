...
\documentclass[openright]{tex/estilos/normas-utf-tex} %openright = o capitulo comeca sempre em paginas impares
...
(\xmark) Autorizo a disponibilização do seguinte correio eletrônico para contato: xxxx@xxx.xx} % Autorização de Divulgação de e-mail para contato. Utilize \cmark na linha que preferir e \xmark na linha que não refletir seu desejo
...
\comentario{\UTFPRdocumentodata\ apresentada como requisito para obtenção do título de ``\UTFPRtitulacaodata\ em Engenharia Elétrica'' do \UTFPRprogramadata\ (\UTFPRprogramaSigladata) da \ABNTinstituicaodata\ (\ABNTinstituicaoSigladata).}
...
\begin{figure}[!htb]
    ...
    \includegraphics[width=5cm]{assets/imagens/dummy.eps} % <- formatos PNG, EPS e PDF
...
Segue agora um exemplo de Seção secundária (\ref{sec:secundaria})

\section{Seção secundária}
\label{sec:secundaria}

Segue agora um exemplo de Seção terciária (\ref{subsec:terciaria})

\subsection{Seção terciária}
\label{subsec:terciaria}

Segue agora um exemplo de Seção quaternária (\ref{subsubsec:quaternaria})

\subsubsection{Seção quaternária}
\label{subsubsec:quaternaria}

Segue agora um exemplo de Seção quinária (\ref{subsubsubsec:quinaria})

\subsubsubsection{Seção quinária}
\label{subsubsubsec:quinaria}

Não existem mais níveis de Seção.
...
\label{bibstart}
\bibliography{tex/bibliografia/reflatex} % geracao automatica das referencias a partir do arquivo reflatex.bib
\label{bibend}
